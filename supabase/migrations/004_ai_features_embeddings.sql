...
CREATE POLICY "Users can manage own document embeddings" ON document_embeddings FOR ALL USING (auth.uid() = user_id);
...
CREATE INDEX idx_case_embeddings_user ON case_embeddings(user_id);
ALTER TABLE case_embeddings ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can manage own case embeddings" ON case_embeddings FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "All users can read case embeddings" ON case_embeddings FOR SELECT USING (TRUE);
...
CREATE POLICY "Users can manage own translations" ON document_translations FOR ALL USING (auth.uid() = user_id);
...
CREATE INDEX idx_case_briefs_case ON case_briefs(case_id);
ALTER TABLE case_briefs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can manage own briefs" ON case_briefs FOR ALL USING (auth.uid() = user_id);
